//! bro — shell prototype. Helium-style chrome, no engine yet.
...
use iced::widget::{button, column, container, mouse_area, operation, row, scrollable, space, stack, svg, text, text_input};
use iced::widget::image;
use iced::{Border, Color, Element, Event, Fill, Shadow, Size, Subscription, Task, Theme, Vector, event, mouse, window};
...
mod titlebar {
    ...
    pub fn init<T: Send + 'static>() -> iced::Task<T> {
        window::latest()
            .and_then(|id| window::run(id, |w| patch(w)))
            .discard()
    }
...
impl Profile {
    fn new(name: &str, color: Color, first_tab_id: usize) -> Self {
        Self { name: name.into(), color, tabs: vec![Tab::new(first_tab_id)], active: 0 }
    }
    fn tab(&self) -> &Tab {
...
impl Browser {
    ...
    fn pump(&mut self) -> Task<Message> {
        ...
        let uploads = frames
            .into_iter()
            .map(|(tab, handle)| image::allocate(handle).map(move |r| Message::Allocated(tab, r.ok())));
        for (id, up) in updates {
            let Some(t) = self.profiles.iter_mut().flat_map(|p| p.tabs.iter_mut()).find(|t| t.id == id) else { continue };
            match up {
                engine::Update::Url(u) => t.set_current(u.to_string()),
                engine::Update::Title(title) => {
                    let fallback = page_title(&t.url);
                    t.title = title.filter(|s| !s.trim().is_empty()).unwrap_or(fallback);
                }
                engine::Update::Load(_) => {}
            }
        }
        Task::batch(uploads)
    }
    ...
    fn t(&self, key: Key2) -> f32 {
        match self.anims.get(&key) {
            ...
            None => if key.0 == "appear" { 1.0 } else { 0.0 },
        }
    }
    ...
    fn animating(&self) -> bool {
        ...
        self.anims.values().any(|a| a.is_animating(n))
            || [&self.theme_a, &self.popup_a, &self.sidebar_a, &self.content_a]
                .iter()
                .any(|a| a.is_animating(n))
    }
...
impl MenuItem {
    const ALL: [MenuItem; 21] = [
        Self::NewTab, Self::NewWindow, Self::Incognito, Self::Divider1,
        Self::History, Self::Downloads, Self::Bookmarks, Self::TabGroups, Self::Extensions, Self::DeleteData, Self::Divider2,
        Self::Zoom, Self::Divider3,
        Self::Print, Self::FindEdit, Self::CastSaveShare, Self::MoreTools, Self::Divider4,
        Self::About, Self::Customize, Self::Settings,
    ];
...
enum Message {
    ...
    Allocated(usize, Option<image::Allocation>),
}
...
fn apply(b: &mut Browser, m: Message) -> Task<Message> {
    match m {
        ...
        Message::UrlSubmit => {
            ...
            b.pm().tab_mut().goto(url);
        }
        Message::FocusOmnibox => return operation::focus(OMNIBOX_ID),
        Message::Back => {
            let id = b.p().tab().id;
            if b.engine.as_ref().is_some_and(|e| e.has(id)) {
                b.engine().back(id);
                return Task::none();
            }
            let t = b.pm().tab_mut();
        ...
        Message::WebKey(down, key, mods) => {
            let deliver = if down { b.keys_down.insert(key.clone()) } else { b.keys_down.remove(&key) };
            let id = b.p().tab().id;
            if let (true, Some(e)) = (deliver, &b.engine) {
                e.key(id, down, &key, mods);
            }
        }
        Message::Allocated(tab, frame) => {
            if let (Some(f), Some(e)) = (frame, b.engine.as_mut()) {
                e.set_frame(tab, f);
            }
        }
        ...
        Message::Menu(item) => {
            ...
            match item {
                ...
                MenuItem::Incognito => {
                    let task = b.open(None);
                    b.pm().tab_mut().incognito = true;
                    return task;
                }
...
fn subscription(b: &Browser) -> Subscription<Message> {
    ...
    let keys = event::listen_with(|e, status, _| {
        ...
        if key.as_ref() == Key::Named(keyboard::key::Named::Escape) {
            return Some(Message::ClosePopup);
        }
        ...
        match key.as_ref() {
            ...
            Key::Character(c) => {
                c.parse::<usize>().ok().filter(|n| (1..=9).contains(n)).map(|n| Message::SelectTab(n - 1)).or_else(to_page)
            }
            _ => to_page(),
        }
    });
...
fn view(b: &Browser) -> Element<'_, Message> {
    ...
    let anchored = container(panel)
        ...
        .align_x(iced::Right)
        .align_y(iced::Top)
        .padding(iced::Padding { top: chrome_h + BASE - 8.0 * (1.0 - t), right: OMNIBOX_MARGIN, bottom: 0.0, left: 0.0 });
...
fn tab_item<'a>(b: &Browser, i: usize, t: &'a Tab, p: Palette) -> Element<'a, Message> {
    ...
    let label = text(&t.title).size(12.5).color(mix(p.muted, p.text, a.max(h * 0.6))).wrapping(text::Wrapping::None);
...
fn toolbar(b: &Browser, p: Palette) -> Element<'_, Message> {
    ...
    let bar = row![
        ...
        icon_btn(b, "puzzle", p, Some(Message::Menu(MenuItem::Extensions))),
    ...
    mouse_area(bar)
        ...
        .on_double_click(Message::ZoomWindow).into()
}
...
fn avatar_btn<'a>(b: &Browser, prof: &Profile, p: Palette) -> Element<'a, Message> {
    ...
    // ring grows on hover
    let size = 20.0 + 2.0 * h;
    let circle = container(text(initial).size(11).color(Color::WHITE))
        .center(size)
        .style(move |_| container::Style {
...
fn profile_menu(b: &Browser, p: Palette, t: f32) -> Element<'_, Message> {
    let mut col = column![
        container(text("Profiles").size(11).color(p.muted)).padding([6, 12]),
    ];
    ...
    col = col.push(menu_row(b, kid("manage profiles"), "settings", "Manage profiles", "›", p, Message::Menu(MenuItem::Settings)));
    panel(col.into(), p, t)
}
...
fn zoom_row(b: &Browser, p: Palette) -> Element<'_, Message> {
    ...
    let small = |g: &'static str, m: Message| {
        ...
        hov(
            k,
            button(container(icon(g, 14.0, p.text)).center(24))
                .padding(0)
                .on_press(m)
                .style(move |_, s| flat(p, h, s)),
        )
...
fn icon_btn<'a>(b: &Browser, glyph: &'static str, p: Palette, on: Option<Message>) -> Element<'a, Message> {
    ...
    let btn = button(container(icon(glyph, 16.0, mix(p.muted, p.text, en))).center(TOOLBAR_BTN))
        .padding(0)
...
fn icon<'a>(name: &str, size: f32, color: Color) -> Element<'a, Message> {
    ...
    let bytes = icons!(
        "app-window", "check", "chevron-left", "chevron-right", "clock-arrow-left", "download", "ellipsis-vertical",
        "info", "layout-grid", "maximize", "minus", "paintbrush", "panel-left", "plus", "printer", "puzzle",
        "rotate-cw", "search", "settings", "share", "square-plus", "star", "sun-moon", "trash", "venetian-mask",
        "wrench", "x", "zoom-in"
    );
